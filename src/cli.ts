...
program
  ...
  .version('1.0.0')
  .option('--stale-months <number>', 'Months since last publish to flag as stale', '12')
  .option('--min-downloads <number>', 'Minimum weekly downloads threshold', '1000')
  .option('--skip-dev', 'Exclude devDependencies from audit', false)
  .option('--output <format>', 'Output format: text or json', 'text')
  .option('--fail-on-risk', 'Exit with non-zero code if risks are found', false)
  .action(async (options) => {
    try {
      ...
      if (isNaN(config.staleMonths) || config.staleMonths < 0) {
        console.error('Error: --stale-months must be a positive number');
        process.exit(1);
      }

      if (isNaN(config.minDownloads) || config.minDownloads < 0) {
        console.error('Error: --min-downloads must be a positive number');
        process.exit(1);
      }

      if (config.outputFormat !== 'text' && config.outputFormat !== 'json') {
        console.error('Error: --output must be either "text" or "json"');
        process.exit(1);
      }
      ...
      console.log(`Fetched metadata for ${registryMetadata.size} packages`);

      // Audit packages
      console.log('🔍 Auditing packages for risks...');
      const auditor = new DependencyAuditor(config);
      const results = await auditor.auditPackages(packages, registryMetadata);
