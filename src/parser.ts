...
import type { PackageInfo, PackageLock, PackageLockDependency } from './types.js';
...
export class DependencyParser {
  ...
  async parseDependencies(skipDev: boolean = false): Promise<PackageInfo[]> {
    ...
    const directDeps = { ...packageJson.dependencies, ...(skipDev ? {} : packageJson.devDependencies) };
  ...
  private readLockFile(): PackageLock | null {
    ...
    try {
      const content = readFileSync(join(this.projectRoot, 'package-lock.json'), 'utf-8');
      return JSON.parse(content);
    } catch {
      // Try yarn.lock (simplified parsing - yarn.lock is more complex)
      try {
        const content = readFileSync(join(this.projectRoot, 'yarn.lock'), 'utf-8');
        // For now, we'll just return null for yarn.lock and rely on package.json
        // A full yarn.lock parser would be more complex
        console.warn('yarn.lock detected but full parsing not implemented. Using package.json only.');
        return null;
      } catch {
        return null;
      }
  ...
  private extractFromLockFile(
    ...
    const traverse = (deps: Record<string, PackageLockDependency>, isDevContext: boolean = false) => {
      for (const [name, dep] of Object.entries(deps)) {
        ...
        if (!packages.has(name) || packages.get(name)!.version !== dep.version) {
          packages.set(name, {
            name,
            version: dep.version,
            isDirect: false,
