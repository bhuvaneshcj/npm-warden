...
export class DependencyAuditor {
  ...
  async auditPackages(
    ...
    for (const pkg of packages) {
      ...
      const risks = this.assessRisks(pkg, metadata, securityVulnerabilities);
  ...
  private assessRisks(
    ...
    if (metadata.lastPublished) {
      const monthsSincePublish = this.getMonthsSince(metadata.lastPublished);
      if (monthsSincePublish > this.config.staleMonths) {
        risks.push({
          type: 'stale',
          severity: monthsSincePublish > 24 ? 'high' : monthsSincePublish > 18 ? 'medium' : 'low',
    ...
    if (metadata.weeklyDownloads !== undefined) {
      if (metadata.weeklyDownloads < this.config.minDownloads) {
        risks.push({
          type: 'low-usage',
          severity: metadata.weeklyDownloads < 100 ? 'high' : metadata.weeklyDownloads < 500 ? 'medium' : 'low',
