  ...
  generateTextReport(results: AuditResult[]): string {
    ...
    for (const result of flaggedResults) {
      for (const risk of result.risks) {
        ...
        if (!byType[risk.type].some(r => r.package.name === result.package.name)) {
    ...
    if (byType.security.length > 0) {
      ...
      for (const result of byType.security) {
        const securityRisk = result.risks.find(r => r.type === 'security');
    ...
    if (byType['low-usage'].length > 0) {
      ...
      for (const result of byType['low-usage']) {
        const lowUsageRisk = result.risks.find(r => r.type === 'low-usage');
    ...
    if (meta) {
      if (meta.lastPublished) {
        const monthsAgo = Math.round(
          (Date.now() - meta.lastPublished.getTime()) / (1000 * 60 * 60 * 24 * 30.44)
        );
  ...
  generateJsonReport(results: AuditResult[]): string {
    ...
    const report = {
      ...
      summary: {
        ...
        byType: {
          security: results.filter(r => r.risks.some(risk => risk.type === 'security')).length,
          stale: results.filter(r => r.risks.some(risk => risk.type === 'stale')).length,
          'low-usage': results.filter(r => r.risks.some(risk => risk.type === 'low-usage')).length,
        },
      },
      packages: flaggedResults.map(result => ({
        ...
        metadata: result.registryMetadata ? {
          lastPublished: result.registryMetadata.lastPublished?.toISOString(),
          weeklyDownloads: result.registryMetadata.weeklyDownloads,
