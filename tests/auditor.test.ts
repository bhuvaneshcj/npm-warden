...
import type { PackageInfo, RegistryMetadata, AuditConfig } from '../src/types.js';
...
test('DependencyAuditor - flag stale packages', async () => {
  ...
  const packages: PackageInfo[] = [
    { name: 'stale-package', version: '1.0.0', isDirect: true, isDev: false },
  ];
...
test('DependencyAuditor - flag low usage packages', async () => {
  ...
  const auditor = new DependencyAuditor(config);
  const packages: PackageInfo[] = [
    { name: 'low-usage-package', version: '1.0.0', isDirect: true, isDev: false },
  ];
...
test('DependencyAuditor - no risks for healthy package', async () => {
  ...
  const auditor = new DependencyAuditor(config);
  const packages: PackageInfo[] = [
    { name: 'healthy-package', version: '1.0.0', isDirect: true, isDev: false },
  ];
